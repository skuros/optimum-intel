...
"""Compatibility hooks for Optimum exporters."""
...
except Exception:  # diffusers not installed or outdated
    import_utils = None

if import_utils is not None:
    class_mapping = getattr(import_utils, "_class_to_module", None)
    targets = {"HunyuanDiTPipeline", "StableDiffusionHunyuanDiTPipeline"}
    if isinstance(class_mapping, dict):
        for cls in targets:
            class_mapping.pop(cls, None)
    else:
        for cls in targets:
            if hasattr(import_utils, cls):
