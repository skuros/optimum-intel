...
def _sanitize_model_export_name(model_name: str) -> str:
    """Normalize submodel names used to create OpenVINO file stems."""
    if model_name.startswith("sam2video_"):
        model_name = model_name[len("sam2video_"):]
    if model_name.startswith("sam2_"):
        model_name = model_name[len("sam2_"):]
    return model_name
...
def export_from_model(
    ...
    **kwargs_shapes,
    ...
    if library_name == "open_clip":
        ...
        files_subpaths = [
            "openvino_" + _sanitize_model_export_name(model_name) + ".xml"
            for model_name in models_and_export_configs.keys()
        ]
...
def _get_sam2_video_submodels_and_export_configs(
    ...
    for component_model_type, component_task in component_specs:
        ...
        models_for_export[component_model_type] = (model, export_config)
